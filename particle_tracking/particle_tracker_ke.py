...
class ElectronCube:
    ...
    # Then can backproject to surface of volume
        ...
        # 1. Project rays to the exit boundary
        self.rf = self.ray_at_exit()

        # -----------------------------------------------------------------
        # NEW: IMPRINT MIDPLANE MASK TO TEST FOCUS
        # -----------------------------------------------------------------
        # Extract the 6D final states to calculate where they crossed the midplane
        x, y, z, vx, vy, vz = self.sf[0], self.sf[1], self.sf[2], self.sf[3], self.sf[4], self.sf[5]
        midplane = self.extent / 2.0
        
        # Track coordinates at the midplane based on probing direction
        if self.probing_direction == 'x':
            t_mid = (x - midplane) / vx
            h_mid = y - vy * t_mid  # Horizontal coordinate at midplane
            v_mid = z - vz * t_mid  # Vertical coordinate at midplane
        elif self.probing_direction == 'y':
            t_mid = (y - midplane) / vy
            h_mid = x - vx * t_mid
            v_mid = z - vz * t_mid
        ...
        # Define mask pattern: circlular beam stop
        # print('h_mid**2:', h_mid**2)
        # print('v_mid**2:', v_mid**2)
        mask = (h_mid**2 + v_mid**2) > (0.02**2) # blocks a 2cm radius circle

        # Apply the mask: set un-passed rays to NaN so downstream routines ignore them
        self.rf = self.rf[:, mask]
        print(f"Rays passing through mask: {self.rf.shape[1]} out of {Np}")
        # -----------------------------------------------------------------

        # Fix amplitudes (if you have downstream code for this, it will now see NaNs for blocked rays)
        return self.rf
    ...
    def imprint_mask(ode_sol, extent, probing_direction):
        ...
        mask = (np.abs(y_at_mid) < 0.1) | (np.abs(z_at_mid) < 0.1)
